...
fn run_session_variant(
    ...
    // Warn if treatment session didn't receive hook injection
    if is_treatment && !metrics.hook_injected {
...
fn write_markdown_report(
    ...
    writeln!(
        md,
        "Each dataset runs every task twice: **control** (Claude Code alone) vs \
         **treatment** (Claude Code + codemap). Both sessions explore the same repo \
         copy in read-only mode (`--permission-mode plan`) and report which files are \
...
fn write_dataset_section(
    ...
    if !paired.is_empty() {
        ...
        writeln!(md, "|------|---------|-------|-----------|--------|------|").unwrap();
        for r in results {
            if let Some(m) = r.control.as_ref().or(r.treatment.as_ref()) {
                let total = m.input_tokens + m.output_tokens;
                writeln!(
                    md,
                    "| {} | {} | {} | {} | {:.1}k | {:.1}s |",
